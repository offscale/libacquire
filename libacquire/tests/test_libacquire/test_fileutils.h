...
TEST x_parse_out_extension_should_be_include_both_dots_when_dot_then_whole_filename_is_extension(void) {
    ASSERT_FALSE(strcmp(get_extension(".tar.gz"), ".tar.gz") != 0);
    PASS();
}

/* Overkill to require this test to pass?
TEST x_parse_out_extension_should_be_include_both_dots_when_whole_filename_is_extension(void) {
...
SUITE (fileutils_suite) {
    ...
    RUN_TEST(x_parse_out_extension_should_be_include_both_dots_when_dot_then_whole_filename_is_extension);
